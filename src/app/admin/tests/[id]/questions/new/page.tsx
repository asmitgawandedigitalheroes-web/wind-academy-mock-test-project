...
import { addQuestionToTest, getTestDetails, uploadQuestionImage } from '@/app/actions/admin'
...
export default function AddQuestionPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (test?.target_questions && (test?.question_count || 0) >= test.target_questions) {
      setError('Question limit has been exceeded. If you want to add more questions, go to test settings and update the question count.')
      setLoading(false)
      return
    }
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
          <div className="flex items-center gap-4 md:gap-5">
            ...
            <div>
              <h1 className="text-2xl md:text-3xl font-black text-[#0f172a] tracking-tight">Add New Question</h1>
              <p className="text-sm md:text-base text-slate-500 font-medium">Create a new question for this test.</p>
            </div>
          </div>
          <button
